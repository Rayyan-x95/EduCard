...
const replacements = [
  ...
  { regex: /magnetic-btn bg-\[#E63B2E\] text-\[#F5F3EE\] px-12 py-5 rounded-full font-mono text-lg font-bold inline-flex items-center gap-4 group shadow-xl shadow-\[#E63B2E\]\/20/g, replacement: "btn-primary px-12 py-5 text-lg shadow-xl shadow-primary/20" },
  // Generator page replacements
...
function processDirectory(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      // Fix residual button artifacts
      content = content.replace(/magnetic-btn bg-primary text-light/g, "btn-primary");
      content = content.replace(/bg-primary animate-pulse/g, "bg-accent animate-pulse");
      content = content.replace(/text-primary uppercase tracking-widest\">Generator Active/g, 'text-accent uppercase tracking-widest\">Generator Active');
